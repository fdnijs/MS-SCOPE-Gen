...
if __name__ == "__main__":
    df = pd.read_csv(f"data/MassSpecGym/MassSpecGym.tsv", sep="\t")
    only_MH = False
    if only_MH:
        save_dir = "MassSpecGym_MH"
    else:
    ...
    with open(f"data/{save_dir}/{save_dir}.mgf", "w") as f:
        f.write(mgf_output)
    ...
    label_df.to_csv(f"data/{save_dir}/{save_dir}_labels.tsv", sep="\t", index=False)