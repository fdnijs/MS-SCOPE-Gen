...
def store_results(data_dir, name, alpha, score, mass_bins,
                  coverages, sizes, first_adms, bins,
                  gt_masses, ratio_cal_test,
                  type=None, std_coverages=None,
                  set_positions_data_idx=None):
    import os
    import pickle
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    # Prepare everything you want to save in one dict:
    result_to_save = {
        "coverages": coverages,
        "sizes": sizes,
        "adm_counts": first_adms,
        "mass_bins": bins,
        "gt_masses": gt_masses,
        "std_coverages": std_coverages,
        "set_positions_data_idx": set_positions_data_idx
    }
    with open(os.path.join(path, f"{name}_{score}{mass_bins}_{alpha_str}.pkl"), 'wb') as file:
